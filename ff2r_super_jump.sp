...
"special_jump"
{
    ...
    "color"            "255 0 0 255"  // Czerwony kolor tekstu
...
void ParseColorString(const char[] colorStr, int color[4])
{
    ...
    color[0] = StringToInt(buffer[0]); // R
    color[1] = StringToInt(buffer[1]); // G
    color[2] = StringToInt(buffer[2]); // B
    color[3] = StringToInt(buffer[3]); // A
}
...
public Action OnPlayerRunCmd(int client, int &buttons)
{
    ...
    else if (holdingM2[client]) 
    {
        ...
        if (g_flSuperJumpCharge[client] >= 100.0 && g_flSuperJumpCooldown[client] <= GetGameTime())
        {
            ...
            float flJumpHeight = 750.0; 
            float flForwardMulti = 1.0; 
            float flVelocity[3];
